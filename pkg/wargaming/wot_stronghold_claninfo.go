...
type WotStrongholdClaninfo struct {
	// Clan ID
	...
	BattlesForStrongholdsStatistics struct {
		...
		LastTime10 UnixTime `json:"last_time_10,omitempty"`
	...
	SkirmishStatistics struct {
		...
		LastTime6 UnixTime `json:"last_time_6,omitempty"`
		// End time of the last battle held on Tier VIII vehicles
		LastTime8 UnixTime `json:"last_time_8,omitempty"`
		// Number of defeats on Tier X vehicles
		Lose10 int `json:"lose_10,omitempty"`
...
//     "uk" &mdash; Ukrainian 
//     "kk" &mdash; Kazakh
func (client *Client) WotStrongholdClaninfo(realm Realm, clanId []int, fields []string, language string) (*WotStrongholdClaninfo, error) {
	...
	var result *WotStrongholdClaninfo
